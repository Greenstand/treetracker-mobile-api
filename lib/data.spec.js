...
describe('The data functions', function () {
  ...
  it('creates a user', function(done){
    data.findOrCreateUser(uuidv1().substring(0,30), 'Test', "Test2", "Test3", function(data){
      expect(data.length).to.not.equal(0);
      done();  
    });
  });
  ...
  it('creates a user with phone number', function(done){
    data.findUser('1234567654', function(data){ // TODO: wipe testing database
      expect(data.length).to.not.equal(0);
      done();  
    });
  });

  it('creates a planter registration', function(done){
    data.createPlanterRegistration(1, 11,
      {
        'first_name' : "Test",
        'last_name' : "User",
  ...
  it('finds a user', function(done){
    data.findUser('085739203636', function(data){
      expect(data.length).to.not.equal(0);
      done();  
    });
  });
