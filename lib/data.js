class Data {
  ...
  async createTree(userId, deviceId, body) {
    ...
    let uuid = body.uuid ? body.uuid : null;

    const getQuery = {
      text: 'SELECT * FROM trees WHERE uuid = $1',
      values: [uuid]
    }
    ...
    const data = await this.pool.query(getQuery)
      .catch(err => {
    ...
    if (data.length === 0) {
      const tree = await this.pool.query(insertQuery)
        .catch(err => {
