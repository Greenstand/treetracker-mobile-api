class Data {
  ...
  async createTree(userId, deviceId, body) {
    ...
    const query = {
      ...
      values: [userId, lat, lon, gpsAccuracy, timestamp, imageUrl, geometry, planterPhotoUrl, planterIdentifier, sequenceId, deviceId, note],
    }
    const tree = await this.pool.query(query)
      .catch(err => {
    ...
    if(body.attributes) {
      for(let attribute of body.attributes){
        const insert = {
          ...
          ($1, $2, $3)
          RETURNING *`,
          values: [tree.id, attribute.key, attribute.value]
        }
        const stored = await this.pool.query(insert);
        tree.attributes.push(stored.rows[0]);
      }
    }
    return tree;
  }
  ...
  async findUser(identifier){
    return await this.findOrCreateUser(identifier, "Anonymoust", "Planter", "");
  }
